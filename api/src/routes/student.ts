...
router.post('/register', async (req: Request, res: Response) => {
  try {
    ...
    if (result.status === 'allocated') {
      sendAllocationNotifications({ student: { full_name, email, phone, reg_number }, centre: result.centre, batch: result.batch, exam_date: result.exam_date, distance_km: result.distance_km }).catch(console.error);
    }
