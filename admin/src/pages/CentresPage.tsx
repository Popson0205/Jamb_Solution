  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: '10px', marginBottom: '16px' }}>
        <input placeholder="Search name or LGA..." value={search} onChange={e => setSearch(e.target.value)} style={{ padding: '8px 12px', border: '1px solid #ddd', borderRadius: '4px', fontSize: '13px', width: '280px' }} />
      ...
      <div style={{ background: 'white', borderRadius: '8px', overflow: 'auto', boxShadow: '0 1px 4px rgba(0,0,0,0.08)' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
          <thead><tr style={{ background: '#006400', color: 'white' }}>
            {['Centre Name','State','LGA','Town','Capacity/Batch','Latitude','Longitude'].map(h => <th key={h} style={{ padding: '10px 12px', textAlign: 'left' }}>{h}</th>)}
          </tr></thead>
          <tbody>
            {filtered.map((c, i) => (
