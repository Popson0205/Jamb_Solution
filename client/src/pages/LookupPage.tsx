  ...
  const lookup = async () => {
    ...
    try {
      ...
      navigate('/result', { state: {
        status: 'allocated',
        centre, allocation,
        batch: { number: allocation.batch_number, arrival: allocation.arrival_time, exam_start: allocation.exam_start, exam_end: allocation.exam_end },
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f0f4f0', fontFamily: 'Arial, sans-serif', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '24px' }}>
      <div style={{ background: 'white', borderRadius: '8px', padding: '32px', maxWidth: '420px', width: '100%', boxShadow: '0 2px 8px rgba(0,0,0,0.08)' }}>
        ...
        <input value={regNumber} onChange={e => setRegNumber(e.target.value)} placeholder="Enter Registration Number" style={{ width: '100%', padding: '10px', border: '1px solid #ddd', borderRadius: '4px', fontSize: '15px', boxSizing: 'border-box' }} onKeyDown={e => e.key === 'Enter' && lookup()} />
