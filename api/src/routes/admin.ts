...
// GET /api/admin/test-email — debug endpoint to test email sending
router.get('/test-email', async (req: Request, res: Response) => {
  ...
  if (!process.env.SENDGRID_API_KEY) {
    return res.json({ ...results, error: 'SENDGRID_API_KEY is not set in environment variables' });
  }
  ...
  try {
    ...
    const info = await transporter.sendMail({
      from: `"${process.env.SENDGRID_FROM_NAME || 'JAMB Test'}" <${process.env.SENDGRID_FROM_EMAIL || 'noreply@jamb.gov.ng'}>`,
      to: 'roqeebateniolailiasu@gmail.com',
      subject: 'JAMB Email Test',
      html: '<p>Test email from JAMB CBT API. If you receive this, email is working.</p>',
    });

    results.attempt = 'success';
    results.messageId = info.messageId;
    results.response = info.response;
  } catch (err: any) {
    results.attempt = 'failed';
